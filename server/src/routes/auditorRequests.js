...
router.get("/my", requireAuth, requireRole("taxpayer"), async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT ar.id, ar.auditor_id, u.name AS auditor_name,
              ar.reason, ar.status, ar.created_at
    ...
    try {
      const result = await pool.query(
        `SELECT ar.id, ar.reason, ar.status, ar.created_at,
                u.id AS taxpayer_id, u.name AS taxpayer_name
         FROM auditor_requests ar
         JOIN users u ON u.id = ar.taxpayer_id
